...
class Conexion extends PDO
{
    ...
    function subirRecursosH($descripcion, $horas, $precio, $id_Alumno)
    {
        $sql = 'INSERT INTO `recursosh` (`descripcion`, `horas`, `precio`, `id_Alumno`) VALUES (?, ?, ?, ?);';
        $stmt = $this->prepare($sql);
        $stmt->bindParam(1, $descripcion, PDO::PARAM_STR);
        $stmt->bindParam(2, $horas, PDO::PARAM_INT);
    ...
    function updateData($nombre, $apellidos, $email, $id)
    {
    ...
    function updatePassword($current, $newpassword, $id)
    {
        $query_combinada = "SELECT password FROM usuarios WHERE id = ?";
        $stmt = $this->prepare($query_combinada);
        $stmt->bindParam(1, $id, PDO::PARAM_STR);
        $stmt->execute();
        $objectUsuario = $stmt->fetch(PDO::FETCH_OBJ);
        if ($objectUsuario && password_verify($current, $objectUsuario->password)) {
